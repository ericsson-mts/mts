...
{	
    ...
    public Operation execute(Runner runner) throws Exception
    {
        ...
        replace(runner, new XMLElementAVPParser(runner.getParameterPool()), TextEvent.Topic.CORE);
    ...
    /**
     * Read the attributes and check the existing with different parameter case
    ...
    {    		
		...
		{	
			/*
			if (!template.equals(oldTempl)) 
			{
				throw new ExecutionException("Stats counter <" + name + "> already exists with different parameters");    		
			}
			*/
		}
		else
    ...
    private void handleCounter(Runner runner, Element  root) throws Exception
    {
        ...
        for (Element actionElement:actionElements)
        {
            ...
            if(actionElementName.equals("increase"))
            {
                ...
                if(-1 != timestamp) StatPool.getInstance().addValue(statKey, value, timestamp);
                else StatPool.getInstance().addValue(statKey, value);
    ...
    private void handleValue(Runner runner, Element root) throws Exception
    {
        ...
        for (Element actionElement:actionElements)
        {
            ...
            if(actionElementName.equals("newValue"))
            {
            	double value = Double.parseDouble((null!=actionElement.attributeValue("value")?actionElement.attributeValue("value"):actionElement.getText()));
                long timestamp = getTimestamp(actionElement);
                if(-1 != timestamp) StatPool.getInstance().addValue(statKeyValue, value, timestamp);
                else StatPool.getInstance().addValue(statKeyValue, value);
    ...
    private void handlePercent(Runner runner, Element root) throws Exception
    {
        ...
        for (Element actionElement:actionElements)
        {
            ...
            else if(actionElementName.equals("incTotal"))
            {
            	double value = Double.parseDouble((null!=actionElement.attributeValue("value")?actionElement.attributeValue("value"):actionElement.getText()));
                long timestamp = getTimestamp(actionElement);
                if(-1 != timestamp) StatPool.getInstance().addValue(statKeyTotal, value, timestamp);
                else StatPool.getInstance().addValue(statKeyTotal, value);
            	GlobalLogger.instance().getSessionLogger().info(runner, TextEvent.Topic.CORE, "Statistic : ", name, "/_total =+", value);
            }
        }                
    }
    ...
    private long getTimestamp(Element element)
    {
        ...
        if(null != millis) timestamp = Long.parseLong(millis);
        else if(null != seconds) timestamp = (long) (Double.parseDouble(seconds) * 1000);
