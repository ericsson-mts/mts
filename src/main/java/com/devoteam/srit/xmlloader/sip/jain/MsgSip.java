...
public class MsgSip extends MsgSipCommon {
    ...
    public byte[] encode() throws Exception {
        return sipMessage.encodeAsBytes(sipMessage.getTopmostVia().getTransport());
    }
    ...
    public Parameter getParameter(String path) throws Exception {
        ...
        try {
            ...
            if (params[0].toLowerCase().startsWith("content")) {
                ...
                ContentParser contentParser = new ContentParser("SIP", content, boundary);
                contentParser.addContentParameter(var, params, path);
                return var;
            }
        } catch (Exception e) {
            GlobalLogger.instance().getApplicationLogger().warn(TextEvent.Topic.PROTOCOL, e, "Exception in setFromMessage, returning empty parameter");
            var = new Parameter();
        }
        return var;
    }
