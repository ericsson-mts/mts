...
public class TextTester
{
    ...
    public TextTester(URI testFilename, String runnerName, List<String> parameterEditable) throws Exception
    {
        ...
        List<Testcase> testcaseList = tester.getTest().getTestcaseList();
        ...
        {
            runner = new TestRunnerSequential(tester.getTest(), enabledTestcaseList, numberToRunList);
        }
        else if("-load".equals(runnerName))
        {
            runner = new TestRunnerLoad(tester.getTest(), enabledTestcaseList);
        }
        ...
        else
        {
            ...
            runner = new TestRunnerSingle(test, testcase, 1);
        }
            ...
            testcaseRunner.addListener(new NotificationListener<Notification<String, RunnerState>>(){
                public void notificationReceived(Notification<String, RunnerState> notification){
                    switch(notification.getData().getState())
                    {
                        ...
                        case FAILED:
                        case INTERRUPTED:
                            System.out.println(notification.getData().getState() + ": " + testcaseRunner.getParent().getName() + " / " + testcaseRunner.getName());
                            try
                            {
